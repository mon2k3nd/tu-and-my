...
import img2 from "@/assets/TVT00864.JPG.asset.json";
import img3 from "@/assets/TVT01258.JPG.asset.json";
import img4 from "@/assets/TVT01234.JPG.asset.json";
import img5 from "@/assets/TVT01222.JPG.asset.json";
import img6 from "@/assets/TVT01187.JPG.asset.json";
import img7 from "@/assets/TVT01136.JPG.asset.json";
import img8 from "@/assets/TVT01066.JPG.asset.json";
import img9 from "@/assets/TVT01057.JPG.asset.json";
import music from "@/assets/leDuong.mp3.asset.json";
...
const photos = [img1.url, img2.url, img3.url, img4.url, img5.url, img6.url, img7.url, img8.url, img9.url];
...
function WeddingInvitation() {
  ...
  return <main className="paper-texture min-h-screen text-foreground">
    ...
    <section id="home" className="relative min-h-[92svh] overflow-hidden bg-foreground">
      <img src={img1.url} alt="Thảo My và Xuân Tú" className="absolute inset-0 h-full w-full object-cover object-[48%_35%] opacity-80" />
    ...
    <section id="invitation" className="mx-auto grid max-w-5xl gap-10 px-6 py-24 md:grid-cols-[.8fr_1.2fr] md:items-center">
      ...
      <button type="button" onClick={() => setLightbox(img2.url)} className="relative ml-auto w-[88%] overflow-hidden rounded-sm film-shadow md:w-full"><img src={img2.url} alt="Ảnh cưới của Thảo My và Xuân Tú" className="aspect-[3/4] w-full object-cover"/><Maximize2 className="absolute bottom-4 right-4 text-primary-foreground" size={20}/></button>
    </section>
    ...
    <section className="relative min-h-[82svh] overflow-hidden bg-foreground"><img src={img9.url} alt="Thảo My và Xuân Tú dưới tấm voan cưới" className="absolute inset-0 h-full w-full object-cover opacity-60"/><div className="absolute inset-0 bg-gradient-to-t from-foreground via-foreground/20 to-transparent"/><div className="relative mx-auto flex min-h-[82svh] max-w-xl flex-col items-center justify-end px-6 pb-20 text-center text-primary-foreground"><Sparkles className="mb-5 text-secondary"/><p className="leading-7 text-primary-foreground/75">Cảm ơn bạn đã đi qua từng trang nhỏ trong câu chuyện của chúng mình. Hẹn gặp nhau, cùng cười thật nhiều và lưu lại một ngày không thể nào quên.</p><h2 className="mt-8 text-5xl">The End</h2><p className="mt-2 text-xs uppercase tracking-[.28em] text-secondary">See you there</p><div className="mt-10 flex items-center gap-4">{qr && <img src={qr} alt="Mã QR thiệp cưới" className="h-24 w-24 rounded-sm"/>}<div className="text-left text-xs leading-6 text-primary-foreground/65">Thảo My & Xuân Tú<br/>03 · 10 · 2026</div></div></div></section>
